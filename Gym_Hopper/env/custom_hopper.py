...
class CustomHopper(MujocoEnv, utils.EzPickle):
    def __init__(self, domain=None):
        ...
        self.original_masses = np.copy(self.sim.model.body_mass[1:])    # Default link masses [0:torso_mass,1:thigh,2:leg,3:foot]        
    ...
    def sample_parameters(self):
        ...
        lower_bound = self.original_masses * 0.5
        upper_bound = self.original_masses * 1.5

        # Copia le masse originali
        new_masses = np.copy(self.original_masses)

        # Non modificare la massa del torso (index 0)
        for i in range(1, len(new_masses)):
            new_masses[i] = np.random.uniform(lower_bound[i], upper_bound[i])
    ...
    def get_parameters(self):
        ...
        body_names = self.sim.model.body_names[1:] # ADDED
        masses = np.array( self.sim.model.body_mass[1:] )
        return body_names,masses
